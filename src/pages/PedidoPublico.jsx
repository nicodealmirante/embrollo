...
export default function PedidoPublico() {
  ...
  const handlePedido = async () => {
    ...
    setSubmitting(false);
    window.location.href = "https://wa.me/message/PJZSJKNE5TMAJ1";
  };
                ...
                <div className="space-y-0">
                  {[
                    ...
                    .map((m, i) => {
                      ...
                      const bgClass = isNegativo ? "bg-red-50 border-red-200" : isPago ? "bg-green-50 border-green-200" : isContado ? "bg-yellow-50 border-yellow-200" : "bg-red-50 border-red-200";
                      const labelClass = isNegativo ? "text-red-700" : isPago ? "text-green-700" : isContado ? "text-yellow-700" : "text-red-700";
                      const label = isNegativo ? "Debe" : isPago ? "Pago" : isContado ? "Contado" : "Debe";
                      ...
                      return (
                        <div key={i} className={`flex items-center justify-between py-2.5 px-3 rounded-lg border mb-1.5 ${bgClass}`}>
                          <div>
                            <p className={`text-xs ${isNegativo && m.referencia === "Aumento 10%" ? "text-gray-300" : "text-muted-foreground"}`}>{moment(m._fecha).format("DD/MM/YY")}</p>
